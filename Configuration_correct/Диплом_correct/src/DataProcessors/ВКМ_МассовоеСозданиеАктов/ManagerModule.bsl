...
Функция ФормированиеАктовДлитльнаяОперация(Период, Адрес, МассивДоговоровБезАктов) Экспорт
	...
	Попытка 
		Для Каждого Договор из МассивДоговоровБезАктов Цикл
			...
			//Замечание 5. В алгоритме автоматического создания актов результат проверки заполнения не учитывается при проведении. Если проверка заполнения не пройдена, то документ не должен проводится.
		    НовыйРеализация.Записать(РежимЗаписиДокумента.Запись);
			Если НовыйРеализация.ПроверитьЗаполнение() Тогда
