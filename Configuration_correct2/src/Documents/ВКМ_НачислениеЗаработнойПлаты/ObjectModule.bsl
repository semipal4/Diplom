...
Процедура СформироватьДвиженияПоОкладу()
	...
	Для Каждого Строка Из Начисления Цикл
		...
		Если Строка.ВидРасчета = ПланыВидовРасчета.ВКМ_ОсновныеНачисления.Оклад Тогда
			...
			ДвижениеОклад.ВидРасчета = ПланыВидовРасчета.ВКМ_ОсновныеНачисления.Оклад;
			ДвижениеОклад.Сотрудник  = Строка.Сотрудник;  
			
			//---Движения НДФЛ по окладу---
			ДвижениеНДФЛ = Движения.ВКМ_Удержания.Добавить();
			ДвижениеНДФЛ.ПериодРегистрации = Дата;
			ДвижениеНДФЛ.БазовыйПериодНачало = Строка.ДатаНачала;    
			ДвижениеНДФЛ.БазовыйПериодКонец = Строка.ДатаОкончания;  		
			ДвижениеНДФЛ.Сотрудник = Строка.Сотрудник; 
			ДвижениеНДФЛ.БазаВидРасчета = ПланыВидовРасчета.ВКМ_ОсновныеНачисления.Оклад; 
			ДвижениеНДФЛ.ВидРасчета = ПланыВидовРасчета.ВКМ_Удержания.НДФЛ_Оклад;
		КонецЕсли;
	...
	Для каждого Строка из Начисления Цикл     
		...
		Если Строка.ВидРасчета = ПланыВидовРасчета.ВКМ_ОсновныеНачисления.Отпуск Тогда
			...
			Пока Шаг <= КоличествоМесяцевВОтпуске Цикл
				...
				//---Движения НДФЛ по отпуску---
				ДвижениеНДФЛ.ПериодРегистрации = Дата;
				ДвижениеНДФЛ.Сотрудник = Строка.Сотрудник;
				ДвижениеНДФЛ.БазаВидРасчета=Строка.ВидРасчета; 
				ДвижениеНДФЛ.ВидРасчета = ПланыВидовРасчета.ВКМ_Удержания.НДФЛ_Отпуск;
...
Процедура РассчитатьОклад()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ВКМ_УдержанияБазаВКМ_ОсновныеНачисления.ПериодРегистрации КАК ПериодРегистрации,
	               ...
	               |	ВКМ_УдержанияБазаВКМ_ОсновныеНачисления.БазаВидРасчета КАК БазаВидРасчета,
	               |	ВКМ_УдержанияБазаВКМ_ОсновныеНачисления.РезультатБаза КАК РезультатБаза,
	               |	ОКР(ВКМ_УдержанияБазаВКМ_ОсновныеНачисления.РезультатБаза * 13 / 100, 2) КАК Результат
	               |ИЗ
	               ...
	               |			,
	               |			Регистратор = &Ссылка
	               |				И БазаВидРасчета = ЗНАЧЕНИЕ(ПланВидовРасчета.ВКМ_ОсновныеНачисления.Оклад)) КАК ВКМ_УдержанияБазаВКМ_ОсновныеНачисления
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	Регистратор,
	               |	НомерСтроки";
...
Процедура РассчитатьОтпуск() 
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |			,
	               |			Регистратор = &Ссылка
	               |				И БазаВидРасчета = ЗНАЧЕНИЕ(ПланВидовРасчета.ВКМ_ОсновныеНачисления.Отпуск)) КАК ВКМ_УдержанияБазаВКМ_ОсновныеНачисления
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	Регистратор,
	               |	НомерСтроки";
...
//--------------
Процедура СформироватьДвиженияПоПремии() //Премия с выработки 
	
	ЗапросПремия = новый Запрос;  
	ЗапросПремия.Текст = "ВЫБРАТЬ
	...
	ЗапросПремия.УстановитьПараметр("Ссылка", Ссылка);
	ЗапросПремия.УстановитьПараметр("НачалоПериода", НачалоМесяца(Дата));
	ЗапросПремия.УстановитьПараметр("КонецПериода",  КонецМесяца(Дата));
	РезультатЗапросаПремия = ЗапросПремия.Выполнить();
	
	ВыборкаДетальныеЗаписиПремия =  РезультатЗапросаПремия.Выбрать();
	//ВыборкаДетальныеЗаписиТЗПремия=РезультатЗапросаПремия.Выгрузить();    
	
	Пока ВыборкаДетальныеЗаписиПремия.Следующий() Цикл
		...
		ДвижениеПремия.Сотрудник = ВыборкаДетальныеЗаписиПремия.Сотрудник; 
		
		ДвижениеПремия.БазовыйПериодНачало = НачалоМесяца(Дата);  
		ДвижениеПремия.БазовыйПериодКонец = НачалоДня(КонецМесяца(Дата)); 
		//ДвижениеПремия.ПериодДействияНачало = НачалоМесяца(Дата);  
		//ДвижениеПремия.ПериодДействияКонец = НачалоДня(КонецМесяца(Дата));   
		
		//Движение = Движения.ВКМ_ДополнительныеНачисления[ВыборкаДетальныеЗаписи.НомерСтроки - 1];
		ДвижениеПремия.Результат = ВыборкаДетальныеЗаписиПремия.СуммаКОплатеПриход ;  
		ДвижениеПремия.База = ?(ВыборкаДетальныеЗаписиПремия.ПроцентОтРабот=0,0,Окр(ВыборкаДетальныеЗаписиПремия.СуммаКОплатеПриход*100/ВыборкаДетальныеЗаписиПремия.ПроцентОтРабот,2)); 
		ДвижениеПремия.Процент = ВыборкаДетальныеЗаписиПремия.ПроцентОтРабот; 
		
		//---Движение НДФЛ с выработки
		ДвижениеНДФЛ = Движения.ВКМ_Удержания.Добавить();
		ДвижениеНДФЛ.ПериодРегистрации = Дата;
		ДвижениеНДФЛ.БазовыйПериодНачало = НачалоМесяца(Дата);    
		ДвижениеНДФЛ.БазовыйПериодКонец = НачалоДня(КонецМесяца(Дата));  		
		ДвижениеНДФЛ.Сотрудник = ВыборкаДетальныеЗаписиПремия.Сотрудник;
		ДвижениеНДФЛ.ВидРасчета = ПланыВидовРасчета.ВКМ_Удержания.НДФЛ_СВыработки;
		ДвижениеНДФЛ.БазаВидРасчета = ПланыВидовРасчета.ВКМ_ДополнительныеНачисления.ПремияСВыработки;
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ВКМ_УдержанияБазаВКМ_ДополнительныеНачисления.ПериодРегистрации КАК ПериодРегистрации,
	               ...
	               |	ОКР(ВКМ_УдержанияБазаВКМ_ДополнительныеНачисления.РезультатБаза * 13 / 100, 2) КАК Результат
	               |ИЗ
	...
	РезультатЗапроса = Запрос.Выполнить();
	ВыборкаДетальныеЗаписи =  РезультатЗапроса.Выбрать();  
	//ВыборкаДетальныеЗаписиТЗ2 =  РезультатЗапроса.Выгрузить();
