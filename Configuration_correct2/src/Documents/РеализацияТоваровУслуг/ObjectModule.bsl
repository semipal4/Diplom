...
Процедура ВыполнитьАвтозаполнение() Экспорт
	...
	//Замечание 2. При заполнении Реализации товаров и услуг в качестве источника данных для заполнения информации об услугах по обслуживанию используется виртуальная таблица Остатки и обороты. Это избыточно, лучше использовать виртуальную таблицу Обороты.
	//Запрос.Текст = "ВЫБРАТЬ
	...
	//Замечание 6. В алгоритме автозаполнения реализации отбор по договору и контрагенту накладывается в секции ГДЕ запроса. Необходимо накладывать в параметрах виртуальной таблицы.
	//Запрос.Текст = "ВЫБРАТЬ
	...
	//               |	И ВКМ_ВыполненныеКлиентуРаботыОбороты.Клиент = &Контрагент"; 
